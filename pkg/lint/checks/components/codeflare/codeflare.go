...
func (c *RemovalCheck) Validate(ctx context.Context, target *check.CheckTarget) (*check.DiagnosticResult, error) {
	...
	if err != nil || managementState == nil {
		...
		return &check.DiagnosticResult{
			...
			Message: "CodeFlare component is not configured in DataScienceCluster",
		}, nil
	}
	...
	if managementStateStr == "Managed" || managementStateStr == "Unmanaged" {
		...
		return &check.DiagnosticResult{
			...
			Severity: &severity,
			Message: fmt.Sprintf(
				"CodeFlare is still enabled (state: %s) but will be removed in RHOAI 3.x",
				managementStateStr,
			),
	...
	return &check.DiagnosticResult{
		Status:  check.StatusPass,
		Message: fmt.Sprintf("CodeFlare is disabled (state: %s) - ready for RHOAI 3.x upgrade", managementStateStr),
		Details: map[string]any{
			"managementState": managementStateStr,
		},
	}, nil
